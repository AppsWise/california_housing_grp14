...
def log_prediction_request(
    ...
    try:
        ...
        prediction_db.log_prediction_request(
            ...
            processing_time_ms=(
                processing_time * 1000 if processing_time else None
            ),
...
def health():
    ...
    try:
        ...
        try:
            ...
            health_data["recent_predictions"] = stats.get(
                "total_predictions", 0
            )
...
def prediction_stats():
    ...
    try:
        ...
        response_data = {
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "statistics": {"last_24_hours": stats_24h, "last_hour": stats_1h},
...
def predict():
    ...
    try:
        ...
        if request.is_json:
            ...
            form = request.form
            if not form:
                error_msg = "Request must be JSON or form data"
                app.logger.warning(
                    f"Invalid request format from {request.remote_addr}"
                )
                log_prediction_request(
                    "/api/predict", {}, status_code=400, error_message=error_msg
                )
            ...
            input_data = {
                ...
                "housing_median_age": form.get("HouseAge"),
                "total_rooms": form.get("AveRooms"),
                "total_bedrooms": form.get("AveBedrms"),
                "population": form.get("Population"),
                "households": form.get("AveOccup"),
                "median_income": form.get("MedInc"),
                "ocean_proximity": form.get("OceanProximity", "INLAND"),
            }
        ...
        missing_fields = [
            field for field in required_fields if not input_data.get(field)
        ]
        if missing_fields:
        ...
        for field in numeric_fields:
            try:
                ...
                app.logger.warning(
                    f"Invalid numeric value for {field}: {input_data.get(field)}"
                )
                log_prediction_request(
                    "/api/predict",
                    input_data,
        ...
        if input_data["ocean_proximity"] not in valid_ocean_proximity:
            error_msg = (
                f"ocean_proximity must be one of: {valid_ocean_proximity}"
            )
            app.logger.warning(
                f"Invalid ocean_proximity: {input_data['ocean_proximity']}"
            )
            log_prediction_request(
                "/api/predict",
                input_data,
        ...
        app.logger.info(
            f"Successful prediction: {prediction} "
    ...
    try:
        if model is None:
            return render_template(
                "index.html", prediction_text="Error: Model not loaded"
            )
        ...
        return render_template(
            "index.html", prediction_text="Error: Could not make prediction"
        )
...
def batch_predict():
    ...
    try:
        ...
        if not request.is_json:
            ...
            app.logger.warning(
                f"Invalid batch request format from {request.remote_addr}"
            )
            log_prediction_request(
                "/api/predict/batch",
                {},
        ...
        if not isinstance(input_data, list):
            ...
            app.logger.warning(
                f"Invalid batch format: expected list, got {type(input_data)}"
            )
            log_prediction_request(
                "/api/predict/batch",
                input_data,
        ...
        if len(input_data) > max_batch_size:
            error_msg = f"Batch size exceeds {max_batch_size} items"
            app.logger.warning(
                f"Batch size {len(input_data)} exceeds limit {max_batch_size}"
            )
            log_prediction_request(
                "/api/predict/batch",
        ...
        for i, item in enumerate(input_data):
            try:
                ...
                if missing_fields:
                    error_msg = f"Item {i}: Missing fields: {missing_fields}"
                ...
                for field in numeric_fields:
                    try:
                        ...
                        error_msg = (
                            f"Item {i}: Field '{field}' must be numeric"
                        )
                        errors.append({"index": i, "error": error_msg})
                        predictions.append(None)
                        continue
                ...
                if item["ocean_proximity"] not in valid_ocean_proximity:
                    error_msg = f"Item {i}: Invalid ocean_proximity"
                    errors.append({"index": i, "error": error_msg})
                    predictions.append(None)
                    continue
        ...
        app.logger.info(
            f"Batch prediction completed: {len(input_data)} items, "
            f"{response_data['successful_count']} successful, "
            f"{response_data['error_count']} errors "
            f"(processing time: {processing_time:.3f}s)"
        )
...
if __name__ == "__main__":
    ...
    app.logger.info("Logs directory: logs/")
    app.logger.info("=" * 50)

    # Log system metrics at startup
    try:
        prediction_db.log_system_metric(
            "app_startup",
            1.0,
            {"version": "1.0.0", "model_loaded": str(model is not None)},
        )
    except Exception as e:
