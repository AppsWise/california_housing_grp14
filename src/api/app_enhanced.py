...
def prepare_input(input_data):
    ...
    # Ensure the correct column order (same as training)
    expected_columns = [
        "longitude",
        "latitude",
        "housing_median_age",
...
def predict():
    ...
    try:
        ...
        try:
            ...
            app.logger.info(
                f"Input validation successful with {len(warnings)} warnings"
            )
            if warnings:
        ...
        app.logger.info(
            f"Prepared input features: {list(prepared_input.columns)}"
        )
        ...
        app.logger.info(
            ...
            f"(processing time: {processing_time_ms/1000:.3f}s)"
        )
...
def predict_batch():
    ...
    try:
        ...
        try:
            ...
            app.logger.info(
                f"Batch validation successful with {len(warnings)} warnings"
            )
        except ValidationError as e:
            error_response = ErrorResponse(
                error="Validation Error",
...
def check_performance():
    ...
    try:
        ...
        response = {
            ...
            "status": (
                "healthy"
                if not (needs_retrain or needs_time_retrain)
    ...
    try:
        ...
        api_data = {
            ...
            "housing_median_age": float(
                form_data.get("housing_median_age", 0)
            ),
...
if __name__ == "__main__":
    ...
    app.logger.info(
        f"Retraining trigger initialized: {retrain_trigger is not None}"
    )
