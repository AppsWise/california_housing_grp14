...
from src.api.schemas import (
    HousingPredictionInput, BatchPredictionInput,
    PredictionResponse, BatchPredictionResponse,
    ErrorResponse, HealthResponse, MetricsResponse
)
from src.models.predict import HousingPredictor, create_prediction_input
from src.utils.config import api_config
from src.utils.logging_config import setup_logging, StructuredLogger
from src.utils.monitoring import metrics_collector, health_checker, performance_monitor
...
def initialize_database():
    ...
    try:
        db_url = api_config.get('database.url', 'sqlite:///monitoring/logs/predictions.db')
...
def health_check():
    ...
    response = HealthResponse(
        ...
        model_version=api_config.get('model.version', '1.0.0') if model else None,
...
def predict():
    ...
    try:
        ...
        if model is None:
            ...
            performance_monitor.record_request((time.time() - start_time_pred) * 1000, 503)
        ...
        error_details = [{"field": str(err['loc'][-1]) if err['loc'] else 'unknown', 
                         "message": err['msg'], "type": err['type']} for err in e.errors()]
...
def predict_batch():
    ...
    try:
        ...
        if model is None:
            ...
            performance_monitor.record_request((time.time() - start_time_batch) * 1000, 503)
            return jsonify(error_response.dict()), 503
        ...
        # Record metrics
        metrics_collector.record_metric('batch_prediction_count', len(predictions))
