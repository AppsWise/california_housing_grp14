...
class HousingPredictionInput(BaseModel):
    ...
    housing_median_age: float = Field(
        ..., ge=0, le=100, description="Median age of houses in the block"
    )
    total_rooms: float = Field(
        ..., ge=1, description="Total number of rooms in the block"
    )
    total_bedrooms: float = Field(
        ..., ge=1, description="Total number of bedrooms in the block"
    )
    ...
    median_income: float = Field(
        ..., ge=0, description="Median income of households in the block"
    )
...
class PredictionResponse(BaseModel):
    ...
    validation_warnings: List[str] = Field(
        default=[], description="Any validation warnings"
    )
...
class BatchPredictionResponse(BaseModel):
    ...
    predictions: List[float] = Field(
        ..., description="List of predicted house prices"
    )
...
class RetrainingRequest(BaseModel):
    ...
    force_retrain: bool = Field(
        default=False, description="Force retraining even if not needed"
    )
