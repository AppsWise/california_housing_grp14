...
class MLOpsPipeline:
    ...
    def __init__(self, config_path: str = "config/pipeline_config.yaml"):
        ...
        self.eda = HousingEDA(data_path=self.config.get('data_path', 'data/housing.csv'))
    ...
    def run_feature_engineering(self) -> str:
        ...
        try:
            ...
            logger.info(f"Feature engineering completed! Dataset saved to {output_path}")
        ...
        try:
            ...
            if data_path is None:
                if 'feature_engineering' in self.results:
                    data_path = self.results['feature_engineering']['output_path']
                else:
    ...
    def run_full_pipeline(self) -> Dict[str, Any]:
        ...
        pipeline_start_time = logger.info("Pipeline started")
        ...
        try:
            ...
            if self.config.get('run_feature_engineering', True):
                ...
                logger.info("Skipping feature engineering (disabled in config)")
    ...
    def generate_pipeline_report(self) -> str:
        ...
        report_lines = [
            ...
            f"- Feature Engineering Enabled: {self.config.get('run_feature_engineering', False)}",
            f"- Model Training Enabled: {self.config.get('run_model_training', False)}",
            "",
        ]
        ...
        if 'eda' in self.results:
            ...
            report_lines.extend([
                ...
                f"- Dataset Shape: {eda.get('structure', {}).get('shape', 'N/A')}",
                f"- Missing Values: {eda.get('missing_values', {}).get('total_missing', 'N/A')}",
                f"- Numeric Features: {len(eda.get('structure', {}).get('numeric_columns', []))}",
                f"- Categorical Features: {len(eda.get('structure', {}).get('categorical_columns', []))}",
                ""
            ])
        ...
        if 'feature_engineering' in self.results:
            ...
            report_lines.extend([
                ...
                f"- Feature Expansion Ratio: {fe.get('final_features', 0) / max(fe.get('original_features', 1), 1):.2f}x",
                ""
            ])
        ...
        if 'model_training' in self.results:
            ...
            report_lines.extend([
                "## Model Training Results",
                f"- Models Trained: {len(mt.get('models', {})) if mt.get('models') else 'N/A'}",
...
def main():
    ...
    parser.add_argument("--eda-only", action="store_true",
                       help="Run only EDA analysis")
    parser.add_argument("--fe-only", action="store_true",
    ...
    try:
        ...
        if args.eda_only:
            ...
            logger.info(f"Pipeline completed! Report available at: {report_path}")
