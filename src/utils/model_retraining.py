...
class ModelRetrainingTrigger:
    ...
    def __init__(
        ...
        self.retrain_interval_hours = self.config.get(
            "retrain_interval_hours", 24
        )

        logger.info("Model retraining trigger initialized")
    ...
    def _start_data_monitoring(self):
        ...
        self.data_observer.schedule(
            DataFileHandler(self), str(self.data_path), recursive=True
        )
    ...
    def _on_new_data_detected(self, file_path: str):
        ...
        try:
            ...
            if len(new_data) >= self.min_new_samples:
                logger.info(
                    f"New data file has {len(new_data)} samples "
                ...
                if self.config.get("enable_auto_retrain", True):
                    self._trigger_retraining(
                        reason="new_data",
                        details=f"New data file: {file_path}",
                    )
                else:
                    logger.info(
                        "Auto-retrain disabled. Manual trigger required."
                    )
            else:
                logger.info(
                    f"New data file has only {len(new_data)} samples "
                    f"(< {self.min_new_samples})"
                )
    ...
    def check_performance_degradation(self) -> bool:
        ...
        try:
            if not os.path.exists(self.logs_db_path):
                logger.warning(
                    f"Predictions database not found: {self.logs_db_path}"
                )
                return False
            ...
            with sqlite3.connect(self.logs_db_path) as conn:
                ...
                if len(recent_predictions) < 10:
                    logger.info(
                        "Insufficient recent predictions for performance check"
                    )
                    return False

                # Simple performance check: look for anomalies
                predictions = pd.to_numeric(
                    recent_predictions["prediction"], errors="coerce"
                )
                ...
                if outlier_ratio > self.performance_threshold:
                    logger.warning(
                        f"High outlier ratio detected: {outlier_ratio:.3f}"
                    )
                    return True

                logger.info(
                    f"Performance check passed. Outlier ratio: {outlier_ratio:.3f}"
                )
                return False
    ...
    def check_time_based_retrain(self) -> bool:
        ...
        if not self.last_retrain_time:
            ...
            if model_file.exists():
                self.last_retrain_time = datetime.fromtimestamp(
        ...
        if hours_since_retrain >= self.retrain_interval_hours:
            logger.info(
                f"Time-based retrain triggered. Hours since last retrain: "
                f"{hours_since_retrain:.1f}"
            )
            return True
            ...
            if success:
                ...
                if self.config.get("notification_email"):
                    self._send_notification(
                        f"Model retrained successfully. Reason: {reason}"
                    )
            else:
        ...
        try:
            with sqlite3.connect(self.logs_db_path) as conn:
                ...
                cursor.execute(
                    ...
                    VALUES (?, ?, ?, ?)
                """,
                    (datetime.now().isoformat(), reason, details, "triggered"),
                )
    ...
    def run_monitoring_cycle(self):
        ...
        if self.check_time_based_retrain():
            self._trigger_retraining(
                "scheduled_retrain", "Time-based retraining schedule"
            )
            return
...
"""
    ...
    logger.info(
        "✅ Monitoring service script created: scripts/monitoring_service.py"
    )
