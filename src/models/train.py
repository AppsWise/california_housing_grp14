...
preprocessor_linear = ColumnTransformer(
    transformers=[
        ...
        (
            "num_pass",
            StandardScaler(),
            list(set(numeric_features) - set(skewed_cols)),
        ),
        (
            "cat",
...
def train_and_track_model(
    model_name, pipeline, model_params, X_train, X_test, y_train, y_test
):
...
with mlflow.start_run(run_name=f"Best_Model_{best_model_name}"):
    ...
    print(
        f"Best model registered in MLflow Model Registry with URI: {model_uri}"
    )
...
print("Best model saved as models/model.pkl for Flask app")
