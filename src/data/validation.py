...
class HousingPredictionInput(BaseModel):
    ...
    population: float = Field(
        ..., description="Population of the area", gt=0.0, example=322.0
    )
    ...
    ocean_proximity: OceanProximity = Field(
        ..., description="Proximity to ocean", example="NEAR BAY"
    )
    ...
    def validate_households_vs_population(cls, v, values):
        ...
        if "population" in values:
            ...
            if avg_household_size > 20.0:
                raise ValueError(
                    "Average household size seems unrealistic "
                    "(>20 people/household)"
                )
            if avg_household_size < 0.5:
                raise ValueError(
                    "Average household size seems "
                    "unrealistic (<0.5 people/household)"
                )
        return v
    ...
    def validate_california_longitude(cls, v):
        ...
        if not (-124.5 <= v <= -114.0):
            raise ValueError(
                ...
                "California bounds (-124.5 to -114.0)"
            )
        return v
    ...
    def validate_california_latitude(cls, v):
        ...
        if not (32.0 <= v <= 42.0):
            raise ValueError(
                "Latitude should be within California bounds (32.0 to 42.0)"
            )
        return v
...
def validate_prediction_input(
    data: dict,
) -> tuple[HousingPredictionInput, List[str]]:
    """
    ...
    try:
        ...
        if validated_data.median_income > 15.0:
            warnings.append(
                "High median income detected - ensure data quality"
            )

        if validated_data.housing_median_age > 50.0:
            warnings.append(
                "Very old housing detected - prediction may be less accurate"
            )

        rooms_per_household = (
            validated_data.total_rooms / validated_data.households
        )
        if rooms_per_household > 10.0:
            warnings.append(
                "High rooms per household ratio - verify data accuracy"
            )
