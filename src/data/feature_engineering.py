...
class FeatureEngineer:
    ...
    def apply_transformations(self, df: pd.DataFrame, skewed_features: List[str] = None) -> pd.DataFrame:
        """
        Apply transformations to skewed features.
        
        Args:
            df (pd.DataFrame): Input dataframe
            skewed_features (List[str]): List of features that need transformation
            ...
            if col in df_copy.columns:
                ...
                # For ordinal categories, use label encoding
                    ...
                    if col not in self.encoders:
                        ...
                        df_copy[f'{col}_encoded'] = self.encoders[col].transform(df_copy[col].astype(str))
                    logger.info(f"Applied label encoding to: {col}")
        
        return df_copy
    
    def scale_features(self, df: pd.DataFrame, numeric_columns: List[str] = None, 
        ...
        if numeric_columns is None:
            ...
            if 'median_house_value' in numeric_columns:
                numeric_columns.remove('median_house_value')  # Don't scale target
        ...
        if hasattr(scaler, 'n_features_in_') and scaler.n_features_in_ == len(numeric_columns):
        ...
        scaled_df = pd.DataFrame(scaled_data, columns=scaled_columns, index=df_copy.index)
    ...
    def select_features(self, df: pd.DataFrame, target_column: str = 'median_house_value',
                       method: str = 'variance', threshold: float = 0.01) -> pd.DataFrame:
        """
        ...
        if method == 'variance':
            ...
            if selector_key not in self.feature_selectors:
                self.feature_selectors[selector_key] = VarianceThreshold(threshold=threshold)
                
                # Fit selector on numeric features only
                numeric_features = df_copy[feature_columns].select_dtypes(include=['int64', 'float64'])
                selected_features = self.feature_selectors[selector_key].fit_transform(numeric_features)
                selected_feature_names = numeric_features.columns[self.feature_selectors[selector_key].get_support()].tolist()
            else:
                numeric_features = df_copy[feature_columns].select_dtypes(include=['int64', 'float64'])
                selected_features = self.feature_selectors[selector_key].transform(numeric_features)
                selected_feature_names = numeric_features.columns[self.feature_selectors[selector_key].get_support()].tolist()
            
            # Keep selected numeric features and all categorical features
            categorical_features = df_copy[feature_columns].select_dtypes(exclude=['int64', 'float64']).columns.tolist()
            final_features = selected_feature_names + categorical_features + [target_column]
    ...
    def create_polynomial_features(self, df: pd.DataFrame, features: List[str] = None, 
    ...
    def engineer_features(self, df: pd.DataFrame, target_column: str = 'median_house_value',
        ...
        if include_transformations:
            skewed_features = ['total_rooms', 'total_bedrooms', 'population', 'households']
            df_engineered = self.apply_transformations(df_engineered, skewed_features)
        ...
        if include_scaling:
            numeric_columns = df_engineered.select_dtypes(include=['int64', 'float64']).columns.tolist()
        ...
        self.feature_names = [col for col in df_engineered.columns if col != target_column]
        
        logger.info(f"Feature engineering completed. Features: {len(self.original_features)} -> {len(self.feature_names)}")
