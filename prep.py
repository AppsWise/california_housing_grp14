...
def clean_data(df):
    ...
    if "total_bedrooms" in df_clean.columns:
        ...
        if missing_bedrooms > 0:
            logger.info(
                f"Filling {missing_bedrooms} missing values in "
                f"total_bedrooms with median"
            )
            median_value = df_clean["total_bedrooms"].median()
            df_clean["total_bedrooms"].fillna(median_value, inplace=True)
    ...
    if "median_house_value" in df_clean.columns:
        ...
        df_clean["median_house_value"] = df_clean["median_house_value"].clip(
            upper=500000
        )
